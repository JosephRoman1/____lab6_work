...
class FareTransfer extends Contract {
    ...
    async GetCustomerLastFare(ctx, lastTxnId) {
        // Your code goes here
        const customerJSON = await ctx.stub.getState(lastTxnId);
        if(!customerJSON || customerJSON.length === 0) {
            throw new Error(`The asset ${lastTxnId} does not exist`);
        }
    ...
    async UpdatePrimaryTransit(ctx, id, newTransitId) {
        ...
        return ctx.stub.putState(id, Buffer.from(stringify(sortKeysRecursive(updatedTransit))));
